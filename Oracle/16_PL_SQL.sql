/*
    ...
    - SQL 문장 내에서 변수의 정의, 조건처리(IF), 반복처리(LOOP, FOR, WHILE) 등을 지원하여 SQL 단점 보완
    ...
    * PL/SQL 구조
    ...
    - [예외처리부(EXCEPTION SECTION)] : EXCEPTION으로 시작,
                        예외 발생시 해결하기 위한 구문을 미리 기술해둘 수 있는 부분
*/
-- 출력 기능 활성화
SET SERVEROUTPUT ON;
...
DECLARE
    EID NUMBER;
    ENAME VARCHAR2(20);
    PI CONSTANT NUMBER :=3.14; -- CONSTANCT : 상수값 지정한다는 뜻
BEGIN
    EID := &번호;
    ENAME := '&이름';
...
-- 문제로 풀어보기 !
-- 레퍼런스 타입의 변수로 EID, ENAME, JCODE, DTITLE, SAL 선언하고,
...
BEGIN
    ...
    LOOP
        IF MOD(NUM,2) = 0 THEN 
            FOR DAN IN 1..9
            LOOP
                DBMS_OUTPUT.PUT_LINE(NUM || ' X ' || DAN || ' = ' || NUM*DAN);
            END LOOP;   
