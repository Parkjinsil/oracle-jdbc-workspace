/*
  * DDL(Data Deficition Language) : 데이터 정의어
...
--> 오류 구문에 제약조건명으로 알려줌!
--> 제약조건 부여시 제약조건명을 지정해주지 않으면 시스템에서 알아서 임의의 제약조건명 부여해버림
...
--> 1은 같지만 아이디까지 묶어서 같은지 인식하므로 중복이라 인식 X

-- 복합키 사용 예시 (어떤 회원이 어떤 상품을 찜하는지에 대한 데이터를 보관하는 테이블)  -> 추천X
...
/*
   * FOREIGN KEY (외래키) 제약조건
   ...
   --> 참조할컬럼명 생략시 참조할테이블명에 PRIMARY KEY 로 지정된 컬럼으로 매칭
*/
DROP TABLE MEM;
CREATE TABLE MEM(
     MEM_NO NUMBER PRIMARY KEY,
...
INSERT INTO MEM VALUES (1, 'user01', 'pass01', '홍길순', '여', null, null, null);
INSERT INTO MEM VALUES (2, 'user02', 'pass02', '김말똥', null, null, null, 10);
INSERT INTO MEM VALUES (3, 'user03', 'pass03', '강개순', '남', null, null,  40); -- 40번이 없어서 연결안됨(부모키 없음)

-- MEN_GRADE(부모테이블) - | -----<- MEM(자식테이블)

--> 이때 부모테이블(MEM_GRADE)에서 데이터값을 삭제할 경우 문제 발생
-- 데이터 삭제 : DELETE FROM 테이블명 WHERE 조건;

--> MEN_GRADE 테이블에서 10번 등급 삭제
...
/*
    ...
    * 삭제옵션 : 부모테이블의 데이터 삭제 시 
                    그 데이터를 사용하고 있는 자식테이블의 값을 어떻게 처리할 건지
