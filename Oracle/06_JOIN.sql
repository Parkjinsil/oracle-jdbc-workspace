  ...
  /*
    1. 오라클 - 등가 조인(EQUAL JOIN) / ANSI - 내부 조인(INNER JOIN / NATURAL JOIN)
     ...
     1) 오라클 전용 구문
        SELECT   컬럼, 컬럼, ..., 컬럼
        FROM    테이블1, 테이블2
        WHERE  테이브1.컬럼명 = 테이블2.컬럼명;
        
        - FROM 정레 조회하고자 하는 테이블들을 콤마로(,) 구분하여 나열
     ...
     2) ANSI 표준 구문
        ...
        JOIN     테이블2 ON ( 테이블1.컬럼명 = 테이블2.컬럼명);
         ...
         - JOIN 절에서 같이 조회하고자 하는 테이블을 기술 후 매칭 시킬 컬럼에 대한 조건을 기술 
           (USING 구문과 ON 구문 사용)
 ...
 -->> 자연조인 구문
 SELECT EMP_ED, EMP_NAME, JOB_NAME, SALARY
 FROM EMPLOYEE
 NATURAL JOIN JOB
 WHERE JOB_NAME = '대리';
 ...
 -- 위 WHERE 절에서 조인 먼저 쓰고 AND 뒤에거 쓴다.
 ...
 -- 8. 보너스를 받지 않는 직원들 중 직급코드가 J4 또는 J7인 직원들의 직원명, 직급명, 급여를 조회
 -- >> ANSI 구문
