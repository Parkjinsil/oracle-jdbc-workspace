...
SELECT TO_CHAR(1234, 'L99') FROM DUAL;
SELECT TO_CHAR(1234, 'L999,999') FROM DUAL;

-- 직원명, 급여, 연봉( 위의 TO_CHAR를 이용해서) 조회
SELECT EMP_NAME, TO_CHAR(SALARY, 'L999,999,999'), TO_CHAR(SALARY*12, 'L999,999,999') AS 연봉
FROM EMPLOYEE
...
SELECT TO_CHAR(SYSDATE, 'DD') -- 월 기준
SELECT TO_CHAR(SYSDATE, 'DDD') -- 년 기준
...
SELECT TO_DATE ('20230321 033823', 'YYYYMMDD HH24MISS') FROM DUAL;
...
SELECT EMP_NAME, TO_CHAR((SALARY+ SALARY*NVL(BONUS,0))*12, 'L999,999,999')
FROM EMPLOYEE;
